...
def multi_scale_deformable_attn_pytorch1(value, value_spatial_shapes,
                                        sampling_locations):
    """CPU version of multi-scale deformable attention.
...
class PartAttention(BaseModule):
    ...
    def forward(self,
        ...
        weights = torch.einsum("bqnc,bqcl->bqnl", output*self.normalize_fact, output.transpose(2, 3))
        weights = F.softmax(weights, dim=-1)
        output1 = torch.einsum("bqnl,bqlc->bqnc", weights, output)
        output1 = output1[:, :, 0, :]
        output1 = output1.permute(1, 0, 2).contiguous().view(num_queries, bs, num_heads * embed_dims)
        return output1